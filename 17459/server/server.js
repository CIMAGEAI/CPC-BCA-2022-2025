...
import serviceAccountKey from "../server/react-js-blog-website-pk-firebase-adminsdk-fbsvc-ebfae2a0df.json" with { type: "json" };
...
server.post("/upload-banner", verifyJWT, (req, res) => {
...
server.post("/create-blog", verifyJWT, async (req, res) => {
    ...
    // Additional validations if it's not a draft
    if (!draft) {
        if (!des?.length || des.length > 200) {
            return res.status(403).json({ error: "Provide a description under 200 characters" });
        }
        ...
        if (!tags?.length || tags.length > 10) {
            return res.status(403).json({ error: "Provide 1 to 10 tags" });
        }
    }
    ...
    // Generate blog_id if not updating
