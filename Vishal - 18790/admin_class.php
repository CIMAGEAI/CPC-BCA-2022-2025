...
function login(){
    ...
    if($qry->num_rows > 0){
        ...
        //  New Logic: Faculty Evaluation Check
        if($login == 2){ // Faculty user
            $uid = $_SESSION['login_id'];
            $check = $this->db->query("SELECT * FROM evaluation_list WHERE faculty_id = $uid");
            if($check->num_rows == 0){
                $_SESSION['force_evaluation'] = true;
            }
        }
    ...
    // Fetch old password if updating existing user
    $old_pass = "";
    if(!empty($id)){
        $check = $this->db->query("SELECT password FROM {$type[$_SESSION['login_type']]} WHERE id = $id");
        $old_pass = $check->num_rows > 0 ? $check->fetch_array()['password'] : "";
    }
    ...
    if(!empty($password)){
        if(strlen($password) < 6){
            ...
            echo json_encode(["status"=>0,"msg"=>"Password must contain at least one number."]);
            exit;
        } elseif(!preg_match('/[^a-zA-Z0-9]/', $password)){
            echo json_encode(["status"=>0,"msg"=>"Password must contain at least one special character."]);
            exit;
        } else {
