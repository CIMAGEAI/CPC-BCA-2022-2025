...
if (adminRequestForm) {
  adminRequestForm.addEventListener('submit', async function(e) {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/borrow/admin-request`, {
        method: 'POST',
        headers: {
...
async function fetchRequestedBooks() {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/borrow?status=pending`, {
      headers: { 'Authorization': 'Bearer ' + token }
    });
...
window.approveRequest = async function(id, btn) {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/borrow/approve/${id}`, {
      method: 'PUT',
      headers: { 'Authorization': 'Bearer ' + token }
    });
    const data = await res.json();
...
window.returnRequest = async function(id) {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/borrow/return/${id}`, {
      method: 'PUT',
      headers: { 'Authorization': 'Bearer ' + token }
    });
    if (!res.ok) {
